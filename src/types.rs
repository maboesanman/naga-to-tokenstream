...
impl TypesDefinitions {
    ...
    fn try_make_type(
        ...
        match &ty.inner {
            ...
            naga::TypeInner::Struct { members, .. } => {
                ...
                self.impls.push(syn::parse_quote! {
                ...
                if has_location_binds {
                    ...
                    self.impls.push(syn::parse_quote! {
                        impl #struct_name {
                            const fn layout(step_mode: wgpu::VertexStepMode) -> wgpu::VertexBufferLayout<'static> {
                            ...
                            pub const fn vertex_layout() -> wgpu::VertexBufferLayout<'static> {
                                Self::layout(wgpu::VertexStepMode::Vertex)
                            }

                            pub const fn instance_layout() -> wgpu::VertexBufferLayout<'static> {
                                Self::layout(wgpu::VertexStepMode::Instance)
                            }
                        }
                    });
                }
    ...
    pub fn definitions(self) -> Vec<syn::Item> {
        ...
        let impls = self.impls
            .into_iter()
            .map(|item_impl| syn::Item::Impl(item_impl));

        items.chain(impls).collect()
    }
}
